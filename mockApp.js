...
// This handles the behavior for 500 http return code to only end the program after 5 consecutive 500 codes are received
function handleFiveErrorEnd(data){
// Added a loop to loop through the 500 http data 5 times   
    for(let i = 0; i<5; i++){
        handleResponse(data); 
    }
// This shutsdown application prevents looping 5 times through the handleHTTPData function
      shutdownApp();
}
...
function handleHTTPData(data) {  
  switch (data.status) {
    ...
    case 401:
      ...
      shutdownApp();
      break;
    //This handles 403 http return codes, 403 should print the message but not kill the function when the api function randomly picks the number
    case 403:
      handleResponse(data);
      break;
    case 404:
